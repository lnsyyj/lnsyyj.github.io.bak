...
ceph-disk工具是在项目需要支持许多不同类型的init系统（upstart, sysvinit, etc…）时创建的，同时能够发现devices。这导致工具最初（以及后来）专注于GPT分区上。特别是在GPT GUID上，它以独特的方式标记设备，以回答以下问题：
...
创建这些不仅很难，而且这些分区要求设备由 Ceph 独占。例如，在某些情况下，在设备加密时将创建一个特殊分区，其中包含未加密的密钥。这是头盘域知识，不会转化为"GPT 分区很简单"的理解。下面是正在创建的特殊分区的示例：
